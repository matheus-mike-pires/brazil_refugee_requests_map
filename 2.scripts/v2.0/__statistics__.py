...
def statistical_analysis(dataframe):
    ...
    plot_global_overview(global_dfs, for_all_groups)
    ...
    while try_again:
        ...
        plot_filtered_overview(filtered_df, groups_made, nationality_chosen_in_filter[0])
                 
        nationality_chosen_in_filter.clear()
        final_opt_now = input('press any key to input other countries. Press q to return to the main menu: \n').lower()
        if final_opt_now == 'q':
            try_again = False
        else:
            continue
